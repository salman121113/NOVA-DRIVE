...
export function FileBrowser({
  ...
  const [selectedIds, setSelectedIds] = useState(new Set() as Set<string>);
  const [isSelectionMode, setIsSelectionMode] = useState(false);
  const timerRef = useRef(null as any); // Using any for Timeout to avoid complex union issues in some environments
                      ...
                      >
                        ...
                        {!isSelectionMode && (
                          <div className="flex items-center gap-1 opacity-100 md:opacity-0 group-hover:opacity-100 transition-opacity">
                            <button 
                              onClick={(e) => { e.stopPropagation(); onToggleStar(folder.id, true); }}
                            ...
                            {currentView === 'trash' ? (
                              <>
                                <button 
                                  onClick={(e) => { e.stopPropagation(); onToggleTrash(folder.id, true); }}
                            ...
                            {!isSelectionMode && (
                              <td className="px-4 md:px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div className="flex items-center justify-end gap-1 md:gap-2 opacity-100 md:opacity-0 group-hover:opacity-100 transition-opacity">
                                  <button 
                                    onClick={(e) => { e.stopPropagation(); onToggleStar(file.id, false); }}
                                  ...
                                  {currentView === 'trash' ? (
                                    <>
                                      <button 
                                        onClick={(e) => { e.stopPropagation(); onToggleTrash(file.id, false); }}
